...
#define NS_LOG_SECTION_RENDER(...) NS_LOG_SECTION(LogSeverity_Info, "Render", __VA_ARGS__)
...
const NsUInt32 AlphaMask = 0x00001;
const NsUInt32 RedMask = 0x00002;
const NsUInt32 GreenMask = 0x00004;
const NsUInt32 BlueMask = 0x00008;
...
{
public:
    GLTexture(GLRenderDevice* device_, NsSize width_, NsSize height_, NsSize levels_,
        TextureFormat::Enum format_, NsBool isInverted_): device(device_), width(width_),
        height(height_), levels(levels_), format(format_), isInverted(isInverted_), object(0),
        state(DefaultSampler) {}

    GLTexture(GLuint object_, NsSize width_, NsSize height_, NsSize levels_,
        TextureFormat::Enum format_, NsBool isInverted_): device(0), width(width_), height(height_),
    ...
    const NsUInt32 width;
    const NsUInt32 height;
    const NsUInt32 levels;
...
{
public:
    GLRenderTarget(GLRenderDevice* device_, NsSize width_, NsSize height_, GLsizei samples_):
...
GLRenderDevice::GLRenderDevice(): mGLVersion(0), mDefaultVAO(0)
{
    ...
    NS_ASSERT(glGetString(GL_VENDOR) != 0 && glGetString(GL_RENDERER) != 0);
    NS_INFO("Vendor:   %s", glGetString(GL_VENDOR));
    NS_INFO("Renderer: %s", glGetString(GL_RENDERER));
...
GLRenderDevice::~GLRenderDevice()
{
    ...
    for (NsSize i = 0; i < NS_COUNTOF(mPrograms); i++)
    {
...
////////////////////////////////////////////////////////////////////////////////////////////////////
Ptr<Texture> GLRenderDevice::WrapTexture(GLuint object, NsSize width, NsSize height, NsSize levels,
...
////////////////////////////////////////////////////////////////////////////////////////////////////
void GLRenderDevice::UnbindObjects() const
{
    ...
    for (NsSize i = 0; i < TextureUnit::Count; i++)
    {
...
////////////////////////////////////////////////////////////////////////////////////////////////////
Ptr<RenderTarget> GLRenderDevice::CreateRenderTarget(const NsChar* label, NsSize width,
    NsSize height, NsSize sampleCount)
{
    NS_UNUSED(label);
    NS_ASSERT(sampleCount > 0);
    NS_LOG_SECTION_RENDER("CreateRenderTarget '%s'", label);

    // samples must be 0 to disable multisampling in glRenderbufferStorageMultisample
    sampleCount = nstl::min_alt(sampleCount, MaxSamples());
    ...
    InitTexture(surface->texture.GetPtr(), format, GL_RGBA, GL_UNSIGNED_BYTE);
...
////////////////////////////////////////////////////////////////////////////////////////////////////
Ptr<RenderTarget> GLRenderDevice::CloneRenderTarget(const NsChar* label, RenderTarget* shared_)
{
    NS_UNUSED(label);
    NS_LOG_SECTION_RENDER("CreateRenderTarget '%s'", label);
    ...
    NsSize width = shared->width;
    NsSize height = shared->height;
...
////////////////////////////////////////////////////////////////////////////////////////////////////
Ptr<Texture> GLRenderDevice::CreateTexture(const NsChar* label, NsSize width, NsSize height,
    NsSize numLevels, TextureFormat::Enum format_)
{
    NS_UNUSED(label);
    NS_LOG_SECTION_RENDER("CreateTexture '%s'", label);
    ...
    InitTexture(texture.GetPtr(), internalFormat, format, type);

    return texture;
}

////////////////////////////////////////////////////////////////////////////////////////////////////
void GLRenderDevice::UpdateTexture(Texture* texture_, NsSize level, NsSize x, NsSize y,
...
////////////////////////////////////////////////////////////////////////////////////////////////////
void GLRenderDevice::BeginRender(NsBool offscreen)
{
    NS_UNUSED(offscreen);
...
////////////////////////////////////////////////////////////////////////////////////////////////////
void GLRenderDevice::BeginTile(const Tile& tile, NsSize surfaceWidth, NsSize surfaceHeight)
{
...
////////////////////////////////////////////////////////////////////////////////////////////////////
void GLRenderDevice::ResolveRenderTarget(RenderTarget* surface, const Tile* tiles, NsSize numTiles)
{
...
////////////////////////////////////////////////////////////////////////////////////////////////////
void* GLRenderDevice::MapVertices(NsSize bytes)
{
...
////////////////////////////////////////////////////////////////////////////////////////////////////
void* GLRenderDevice::MapIndices(NsSize bytes)
{
...
void GLRenderDevice::DrawBatch(const Batch& batch)
{
    ...
    NsUInt8 vertexFormat = mPrograms[batch.shader.v].vertexFormat;
    ...
    const GLvoid *indices = (GLvoid*)(NsUIntPtr)(mDynamicIB.drawPos + 2 * batch.startIndex);
...
void GLRenderDevice::DeleteTexture(GLTexture* texture)
{
    ...
    if (object != 0)
    {
        for (NsSize i = 0; i < NS_COUNTOF(mBoundTextures); i++)
        {
...
////////////////////////////////////////////////////////////////////////////////////////////////////
void GLRenderDevice::CreateBuffer(DynamicBuffer& buffer, GLenum target, NsSize size) const
{
...
////////////////////////////////////////////////////////////////////////////////////////////////////
void GLRenderDevice::SetVertexFormat(NsUInt8 format, NsUIntPtr offset)
{
...
#endif
    {
        ...
        GLenum uvType = mCaps.halfFloatUV? GL_HALF_FLOAT : GL_FLOAT;
        GLsizei uvSize = mCaps.halfFloatUV? 4 : 8;

        if ((format & VFTex0) > 0)
        {
            VD(glVertexAttribPointer(Attr::Tex0, 2, uvType, GL_FALSE, stride, (void*)offset));
            offset += uvSize;
        }

        if ((format & VFTex1) > 0)
        {
            VD(glVertexAttribPointer(Attr::Tex1, 2, uvType, GL_FALSE, stride, (void*)offset));
            offset += uvSize;
        }
...
////////////////////////////////////////////////////////////////////////////////////////////////////
void GLRenderDevice::EnableVertexAttribArrays(NsUInt32 state)
{
    if (mEnabledVertexAttribArrays == 0xffffffff)
    {
        ...
        if (mDefaultVAO == 0)
        {
            ...
            for (NsSize i = Attr::Count; i < mMaxVertexAttribs; i++)
            {
    ...
    NsUInt32 delta = state ^ mEnabledVertexAttribArrays;
    if (delta != 0)
    {
        for (NsSize i = 0; i < Attr::Count; i++)
        {
            NsUInt32 mask = 1 << i;
            if (delta & mask)
            {
...
////////////////////////////////////////////////////////////////////////////////////////////////////
void* GLRenderDevice::MapBuffer(DynamicBuffer& buffer, NsSize size)
{
...
{
#if NS_RENDERER_OPENGL
    NsBool isGL31 = mGLVersion >= NS_GL_VER(3,1);
...
void GLRenderDevice::CreateVertexFormats()
{
    ...
    if (IsSupported(Extension::ARB_vertex_attrib_binding))
    {
        ...
        for (NsSize i = 0; i < NS_COUNTOF(mVertexFormats); i++)
        {
            ...
            if ((i & VFTex0) > 0)
            {
                ...
                V(glVertexAttribFormat(Attr::Tex0, 2, uvType, GL_FALSE, offset));
                offset += uvSize;
            }
            ...
            if ((i & VFTex1) > 0)
            {
                ...
                V(glVertexAttribFormat(Attr::Tex1, 2, uvType, GL_FALSE, offset));
                offset += uvSize;
            }
...
#endif
    {
        for (NsSize i = 0; i < NS_COUNTOF(mVertexFormats); i++)
        {
            ...
            stride += (i & VFTex0) > 0 ? mCaps.halfFloatUV? 4 : 8 : 0;
            stride += (i & VFTex1) > 0 ? mCaps.halfFloatUV? 4 : 8 : 0;
...
{
#if NS_RENDERER_OPENGL
    NsBool haveS8 = IsSupported(Extension::ARB_ES3_compatibility);
    GLenum format = haveS8 ? GL_STENCIL_INDEX8 : GL_DEPTH24_STENCIL8;
#else
    GLenum format = GL_STENCIL_INDEX8;
#endif

    NS_LOG_RENDER("Stencil %d x %d %dx (0x%x)", width, height, samples > 0 ? samples : 1, format);
...
GLuint GLRenderDevice::CreateColor(GLsizei width, GLsizei height, GLsizei samples) const
{
    ...
    NS_LOG_RENDER("Color %d x %d %dx (0x%x)", width, height, samples, format);
    return RenderbufferStorage(samples, format, width, height);
}
...
////////////////////////////////////////////////////////////////////////////////////////////////////
void GLRenderDevice::Resolve(GLRenderTarget* target, const Tile* tiles, NsSize numTiles)
{
...
////////////////////////////////////////////////////////////////////////////////////////////////////
NsBool GLRenderDevice::HaveBlitFramebuffer() const
{
...
////////////////////////////////////////////////////////////////////////////////////////////////////
NsBool GLRenderDevice::HaveBorderClamp() const
{
...
////////////////////////////////////////////////////////////////////////////////////////////////////
NsBool GLRenderDevice::HaveTexStorage() const
{
...
////////////////////////////////////////////////////////////////////////////////////////////////////
NsBool GLRenderDevice::HaveTexRG() const
{
#ifdef NS_PLATFORM_ANDROID
    return false;
#else
    ...
    GLenum type)
{
    ...
    NsSize width = texture->width;
    NsSize height = texture->height;

    NS_LOG_RENDER("Texture %d x %d x %d (0x%x)", width, height, levels, internalFormat);

    if (!HaveTexStorage())
    {
        for (NsSize i = 0; i < levels; i++)
        {
            V(glTexImage2D(GL_TEXTURE_2D, i, internalFormat, width, height, 0, format, type, 0));
            width = nstl::max_alt(1U, width / 2);
            height = nstl::max_alt(1U, height / 2);
        }
    }
    else
    {
...
////////////////////////////////////////////////////////////////////////////////////////////////////
void GLRenderDevice::SetTexture(NsSize unit, GLTexture* texture, SamplerState state)
{
    NS_ASSERT(unit < NS_COUNTOF(mBoundTextures));
    NS_ASSERT(texture->object == 0 || glIsTexture(texture->object));

    NsBool cached = mBoundTextures[unit] == texture->object;
...
////////////////////////////////////////////////////////////////////////////////////////////////////
void GLRenderDevice::ActiveTexture(NsSize unit)
{
...
////////////////////////////////////////////////////////////////////////////////////////////////////
GLuint GLRenderDevice::CompileShader(GLenum type, const NsChar* source) const
{
...
////////////////////////////////////////////////////////////////////////////////////////////////////
GLuint GLRenderDevice::LinkProgram(GLuint vso, GLuint pso, const NsChar** attribs) const
{
...
////////////////////////////////////////////////////////////////////////////////////////////////////
void GLRenderDevice::BindTextureLocation(GLuint program, const NsChar* name, GLint unit)
{
...
////////////////////////////////////////////////////////////////////////////////////////////////////
void GLRenderDevice::Scissor(NsUInt32 x, NsUInt32 y, NsUInt32 width, NsUInt32 height)
{
...
////////////////////////////////////////////////////////////////////////////////////////////////////
void GLRenderDevice::Viewport(NsSize width, NsSize height)
{
...
////////////////////////////////////////////////////////////////////////////////////////////////////
void GLRenderDevice::SetClearColor(NsUInt32 color)
{
    if (mClearColor != color)
    {
        NsFloat32 r = ((color >> 16) & 0xff) / 255.0f;
        NsFloat32 g = ((color >> 8) & 0xff) / 255.0f;
        NsFloat32 b = ((color >> 0) & 0xff) / 255.0f;
        NsFloat32 a = ((color >> 24) & 0xff) / 255.0f;
...
////////////////////////////////////////////////////////////////////////////////////////////////////
void GLRenderDevice::SetClearStencil(NsUInt32 stencil)
{
...
void GLRenderDevice::BindAPI()
{
    ...
    const NsChar* oglPath = "/System/Library/Frameworks/OpenGL.framework/Versions/Current/OpenGL";
...
void GLRenderDevice::DetectGLVersion()
{
    NsUInt32 major = 0;
    NsUInt32 minor = 0;
    const NsChar* version = (const NsChar*)glGetString(GL_VERSION);
    NS_ASSERT(version != 0);
    ...
    {
        NS_ASSERT(false);
        NS_WARNING("Unable to parse GL_VERSION '%s'", version);
    }

    NS_INFO("Version:  %d.%d (%s)", major, minor, version);
#else
    ...
    NS_INFO("Version:  ES %d.%d (%s)", major, minor, glGetString(GL_VERSION));
#endif
...
void GLRenderDevice::FindExtensions()
{
    ...
    const Extension_ Extensions[Extension::Count] =
    {
        ...
        { "GL_ARB_half_float_vertex",               GL_CORE(3,0) + GL_ES_CORE(3,0) },
        ...
        { "GL_OES_vertex_half_float",               GL_CORE(0,0) + GL_ES_CORE(0,0) },
    ...
    {
        NS_LOG_SECTION_RENDER("Extensions");
        ...
        if (mGLVersion >= NS_GL_VER(3,0))
        {
            ...
            {
                const NsChar* extension = (const NsChar*)glGetStringi(GL_EXTENSIONS, i);

                for (NsSize j = 0; j < Extension::Count; j++)
                {
                    if (String::Compare(extension, Extensions[j].name) == 0)
                    {
        ...
        {
            const NsChar* extensions = (const NsChar*)glGetString(GL_EXTENSIONS);

            if (extensions != 0)
            {
                for (NsSize i = 0; i < Extension::Count; i++)
                {
        ...
        for (NsSize i = 0; i < Extension::Count; i++)
        {
            ...
            NsUInt32 coreInVersion = Extensions[i].coreInVersion;
            if (!mExtensions[i].supported && coreInVersion != 0 && mGLVersion >= coreInVersion)
            {
                mExtensions[i].supported = true;
                NS_LOG_RENDER("[+] %s", Extensions[i].name);
            }
            else
            {
                NS_LOG_RENDER("[%c] %s", mExtensions[i].supported ? '*' : ' ', Extensions[i].name);
            }
        }
    }
...
////////////////////////////////////////////////////////////////////////////////////////////////////
NsBool GLRenderDevice::IsSupported(Extension::Enum extension) const
{
    return mExtensions[extension].supported;
}

////////////////////////////////////////////////////////////////////////////////////////////////////
NsSize GLRenderDevice::MaxSamples() const
{
    if (IsSupported(Extension::EXT_multisampled_render_to_texture))
    {
...
#if USE_FLOAT16
    mCaps.halfFloatUV = IsSupported(Extension::ARB_half_float_vertex) ||
        IsSupported(Extension::OES_vertex_half_float),
#endif
...
////////////////////////////////////////////////////////////////////////////////////////////////////
GLint GLRenderDevice::GetInteger(GLenum name, NsSize i) const
{
    NS_ASSERT(i < 4);
    GLint vv[4];
    VD(glGetIntegerv(name, vv));
...
////////////////////////////////////////////////////////////////////////////////////////////////////
GLfloat GLRenderDevice::GetFloat(GLenum name, NsSize i) const
{
    NS_ASSERT(i < 4);
    GLfloat vv[4];
    VD(glGetFloatv(name, vv));
...
////////////////////////////////////////////////////////////////////////////////////////////////////
GLboolean GLRenderDevice::GetBoolean(GLenum name, NsSize i) const
{
    NS_ASSERT(i < 4);
    GLboolean vv[4];
