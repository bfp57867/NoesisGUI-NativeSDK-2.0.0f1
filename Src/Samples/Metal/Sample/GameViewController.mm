...
//  Copyright © 2016 Noesis Technologies. All rights reserved.
//
...
Ptr<IView> g_XamlView;
Ptr<MTLRenderDevice> g_Device;

void NoesisErrorHandler(const NsChar* file, NsSize line, const NsChar* desc, NsBool fatal)
{
    NS_UNUSED(file);
    NS_UNUSED(line);
    NS_UNUSED(fatal);
    NSLog(@"NoesisError: %s", desc);
}
...
- (void)_noesisInit
{
    ...
    Noesis::GUI::SetResourceProvider([path UTF8String]);
    ...
    g_XamlView->SetSize(self.view.bounds.size.width, self.view.bounds.size.height);

    g_Device = *new MTLRenderDevice(_device, _view.colorPixelFormat, _view.depthStencilPixelFormat,
        _view.depthStencilPixelFormat, (NsSize)_view.sampleCount);
    Ptr<VGContext> context = Noesis::GUI::CreateVGContext(g_Device.GetPtr(), VGOptions());
...
#ifdef NOESIS_GUI
    static double startT = [[NSDate date] timeIntervalSince1970];
    double t = [[NSDate date] timeIntervalSince1970] - startT;
    g_XamlView->Update(t);

    IRenderer* renderer = g_XamlView->GetRenderer();
    renderer->UpdateRenderTree();

    if (renderer->NeedsOffscreen())
    {
        g_Device->SetOffScreenCommandBuffer(commandBuffer);
...
#ifdef NOESIS_GUI
        g_Device->SetOnScreenEncoder(renderEncoder);
        renderer->Render();
#endif
